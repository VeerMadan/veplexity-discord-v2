# 🤖 VePlexity Bot — Full Slash Commands Directory

> **Total Commands:** `94`  
> **Prefix:** `/` (Discord Slash Commands)  
> **Bot ID:** `1470533218376613908`
...
*All actions automatically log a case in the database and post rich embeds to the configured `#mod-logs` channel.*

| Command | Arguments | Description |
| :--- | :--- | :--- |
| `/warn` | `<user>` `<reason>` | Issues an official warning (Escalates: 3 = 10m timeout, 4 = 1h, 5 = 12h, 6 = kick, 7 = ban). |
...
*Sends anime reaction GIFs and direct bold interaction messages.*

| Command | Arguments | Description |
| :--- | :--- | :--- |
| `/flirt` | `<user>` | AI-generated charismatic flirty message directed at someone. |
| `/pickup` | `[user]` | Drops a smooth or cheesy pickup line from our curated vault. |
...
## 🎮 Fun, Social & Interactive Games (27 Commands)
*Interactive games, AI entertainment, and server engagement tools.*
...
| `/joke` | *None* | Fresh, funny joke with spoiler tag punchline delivery. |
| `/meme` | *None* | Fetches a fresh, trending meme directly from Reddit (`r/memes`). |
...
| `/chatbot` | `<mode>` (`on`/`off`) | Toggle AI Chatbot mode for `@mention` conversations in the server. |
...
## ⚙️ Utility & Server Information (8 Commands)
*System diagnostics, server stats, user info, and productivity utilities.*

| Command | Arguments | Description |
| :--- | :--- | :--- |
| `/test` | *None* | System diagnostics dashboard showing gateway latency, uptime, memory, and Node version. |
...
| `/rate` | `<thing>` | Rates anything on a 0–10 scale with a filled visual progress bar. |
...
## 🎵 Music Playback Commands (13 Commands)
*YouTube & Spotify streaming with 24/7 VC lock mode.*
